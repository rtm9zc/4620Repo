...
%type <nType>	ExtendsClause
%type <nTypeList> ImplementBlock IdentifierList
%type <expr>	OptionalExpr Expr Constant
%type <iStmt>	IfStmt
%type <wStmt>	WhileStmt
%type <fStmt>	ForStmt
...
%type <lValue>	LValue
%type <call>	Call
%type <arExpr>	ArithmeticExpr
%type <rExpr> 	RelationalExpr
...
ClassDecl:     T_Class T_Identifier ExtendsClause ImplementBlock '{' FieldList '}'
				    { $$ = new ClassDecl(new Identifier(@2, $2), $3, $4, $6); }
;

ExtendsClause: 	T_Extends T_Identifier	{ $$ = new NamedType(new Identifier(@2, $2)); }
	|	/* empty */	{ $$ = NULL; }
;

ImplementBlock:	T_Implements IdentifierList	{ $$ = $2; }
...
IfStmt:	T_If '(' Expr ')' Stmt %prec ELSECHECK	{ $$ = new IfStmt($3, $5, NULL); }
	|	T_If '(' Expr ')' Stmt ElseStmt { $$ = new IfStmt($3, $5, $6); }
;
